...
app.get("/:customListName",function(req,res){
    ...
    List.findOne({title:customListName},function(err,foundList){
        if(!err){
            if(!foundList || foundList.items.length==0){
                if(!foundList){
                    ...
                    newList.save();
                    console.log("Doesn't exist!",customListName);
                }else{
                    List.findOneAndUpdate({title:customListName},{items:defaultItems},function(err){
                        if(err){
                            console.log(err);
                        }else{
                            console.log("Success!");
                ...
                res.render("index",{title:foundList.title,items:foundList.items});
            }
        }
    });
});
